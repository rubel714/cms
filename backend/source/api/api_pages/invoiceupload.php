...
//         )
			...
			foreach ($mergedRows as $mRow) {
				...
				$q = new insertq();
				$q->table = 't_invoiceitems';
				$q->columns = ['InvoiceId', 'AccountCode','CustomerName', 'AccountingPeriod', 'DebitCredit', 'Description', 'JournalType','BaseAmountWithoutVat','VatAmount', 'BaseAmount', 'TransactionDate', 'TransactionReference', 'AnalysisCode1', 'AnalysisCode3', 'AnalysisCode9', 'TransactionAmount','ExchangeRate', 'CurrencyCode','GeneralDescription9', 'GeneralDescription11', 'GeneralDescription14', 'GeneralDescription17', 'GeneralDescription20','SunAccount','Agent','CustomerUserId','OriginalBaseAmountWithoutVat','OriginalVatAmount','OriginalBaseAmount','OriginalTransactionAmount'];
				$q->values = ['[LastInsertedId]', $mRow['AccountCode'], $mRow['CustomerName'], $mRow['AccountingPeriod'], $mRow['DebitCredit'], $mRow['Description'], $mRow['JournalType'], $mRow['BaseAmountWithoutVat'], $mRow['VatAmount'], $mRow['BaseAmount'], $mRow['TransactionDate'], $mRow['TransactionReference'], $mRow['AnalysisCode1'], $mRow['AnalysisCode3'], $mRow['AnalysisCode9'], $mRow['TransactionAmount'], $ExchangeRate, $mRow['CurrencyCode'], $GeneralDescription9, $mRow['GeneralDescription11'], $mRow['GeneralDescription14'], $mRow['GeneralDescription17'], $mRow['GeneralDescription20'], $mRow['SunAccount'], $mRow['Agent'], $mRow['CustomerUserId'], $mRow['BaseAmountWithoutVat'], $mRow['VatAmount'], $mRow['BaseAmount'], $mRow['TransactionAmount']];
				$q->pks = ['InvoiceItemId'];
				$q->bUseInsetId = false;
				$q->build_query();
				$aQuerys[] = $q;
